...
![Рисунок 4.1. Пример массива](../_images/04/array_representation.png)

_Рисунок 4.1. Пример массива_
...
**Массив в PHP** - это упорядоченная структура данных, представляющая собой коллекцию элементов, связанных по принципу "ключ - значение".
...
- С помощью функции `array()` - данный вариант считается _устарелым_ и используется реже в современных проектах.

_Создание индексированного массива_

```php
// Создание массива с помощью array()
// Является устаревшим синтаксисом, но все еще поддерживается
$arr = array(1, 2, 3);
...
> [!TIP]
>
> В браузере переводы строк в выводе `print_r()` не отображаются. Чтобы сохранить форматирование, вывод следует обернуть в тег `<pre>`:
...
Строковые ключи на автоматическую нумерацию не влияют: после элемента `'code'` нумерация продолжается с того места, где она остановилась.
...
Цикл `foreach` предназначен специально для перебора массивов[^2]. Он последовательно выполняет тело для каждого элемента и не требует ни счётчика, ни знания количества элементов.
...
Индексированный массив можно перебрать и циклом `for`. Для этого необходимо знать количество элементов, которое возвращает функция `count()`[^3].
...
Запись `$array[] = $value;` покрывает самый частый случай добавления элемента. Для остальных случаев PHP предоставляет функции, работающие с началом и концом массива, и конструкцию для удаления по ключу[^4].
...
После удаления ключи оставшихся элементов не пересчитываются: в массиве остаются ключи `0` и `2`, а ключ `1` отсутствует. Для получения непрерывной нумерации используется функция `array_values()`.
...
- При объединении оператор `+` добавляет все элементы из `$fruits1` в результирующий массив.
- Поскольку ключи пересекаются, элементы из `$fruits2` игнорируются.
...
`...` - spread-оператор, который позволяет "распаковать" массив в месте его использования, например, при объединении массивов.
...
Функция `implode()` объединяет элементы массива в строку, вставляя между ними разделитель[^6].
...
Функция `array_column()` извлекает из многомерного массива значения одного столбца[^7].
...
Функции сортировки PHP[^8] различаются по трём признакам, и выбор функции сводится к ответу на три вопроса:
...
[^1]: PHP Documentation (2026). _Arrays_. Available at: https://www.php.net/manual/en/language.types.array.php

[^2]: PHP Documentation (2026). _foreach_. Available at: https://www.php.net/manual/en/control-structures.foreach.php

[^3]: PHP Documentation (2026). _count_. Available at: https://www.php.net/manual/en/function.count.php

[^4]: PHP Documentation (2026). _Array Functions_. Available at: https://www.php.net/manual/en/ref.array.php

[^5]: PHP Documentation (2026). _array_merge_. Available at: https://www.php.net/manual/en/function.array-merge.php

[^6]: PHP Documentation (2026). _implode_. Available at: https://www.php.net/manual/en/function.implode.php

[^7]: PHP Documentation (2026). _array_column_. Available at: https://www.php.net/manual/en/function.array-column.php

[^8]: PHP Documentation (2026). _Sorting Arrays_. Available at: https://www.php.net/manual/en/array.sorting.php
